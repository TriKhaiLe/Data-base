...
CREATE DATABASE QUANLIGIAOVU_2022
GO
-- Refesh lai CSDL moi lam tiep cac lenh sau
USE QUANLIGIAOVU_2022
GO
-------------------------------------------------
-------------------------------------------------
...
--III--
--1 
--In ra danh sách (mã học viên, họ tên, ngày sinh, mã lớp) 
--lớp trưởng của các lớp.

SELECT MAHV, HO, TEN, NGSINH, HOCVIEN.MALOP FROM HOCVIEN, LOP
WHERE HOCVIEN.MAHV = LOP.TRGLOP

--2
--In ra bảng điểm khi thi 
--(mã học viên, họ tên , lần thi, điểm số) 
--môn CTRR của lớp “K12”, 
--sắp xếp theo tên, họ học viên.

SELECT HOCVIEN.MAHV, HO, TEN, LANTHI, DIEM FROM HOCVIEN, KETQUATHI, MONHOC, GIANGDAY
WHERE MONHOC.MAMH = KETQUATHI.MAMH 
AND KETQUATHI.MAHV = HOCVIEN.MAHV 
AND KETQUATHI.MAMH = GIANGDAY.MAMH 
AND GIANGDAY.MALOP = 'K12'
ORDER BY TEN ASC, HO ASC


--3
--In ra danh sách những học viên (mã học viên, họ tên) 
...
--4
--In ra danh sách học viên (mã học viên, họ tên) 
--của lớp “K11” thi môn CTRR không đạt (ở lần thi 1).

SELECT HV.MAHV, HO, TEN 
FROM HOCVIEN HV JOIN KETQUATHI KQT ON HV.MAHV = KQT.MAHV
JOIN MONHOC MH ON KQT.MAMH = MH.MAMH
JOIN GIANGDAY GD ON KQT.MAMH = GD.MAMH
JOIN LOP L ON GD.MALOP = L.MALOP
WHERE LANTHI = 1 
AND KQUA = 'Khong dat' 
AND TENMH = 'Cau truc roi rac' 
AND L.MALOP = 'K11'

--5
--Danh sách học viên (mã học viên, họ tên) 
--của lớp “K” 
--thi môn CTRR 
...
JOIN MONHOC MH ON KQT.MAMH = MH.MAMH
JOIN GIANGDAY GD ON KQT.MAMH = GD.MAMH
WHERE GD.MALOP LIKE 'K%' 
AND TENMH = 'Cau truc roi rac' 
AND KQUA = 'Khong dat'

--6
--Tìm tên những môn học 
--mà giáo viên có tên “Tran Tam Thanh” 
...
--10. Môn “Cau Truc Roi Rac” là môn bắt buộc 
--phải học liền trước những môn học (mã môn học, tên môn học) nào.
